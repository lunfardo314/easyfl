...
func TestTupleHasDuplicates(t *testing.T) {
	lib := NewBaseLibrary[any]()

	t.Run("empty tuple", func(t *testing.T) {
		// Empty tuple has no duplicates - returns nil (falsey)
		emptyTuple := tuples.EmptyTupleEditable().Tuple()
		src := fmt.Sprintf("tupleHasDuplicates(0x%s)", hex.EncodeToString(emptyTuple.Bytes()))
		ret, err := lib.EvalFromSource(nil, src)
		require.NoError(t, err)
		require.True(t, len(ret) == 0, "empty tuple should have no duplicates")
	})

	t.Run("single element", func(t *testing.T) {
		// Single element has no duplicates
		tup := tuples.MakeTupleFromDataElements([]byte{1, 2, 3})
		src := fmt.Sprintf("tupleHasDuplicates(0x%s)", hex.EncodeToString(tup.Bytes()))
		ret, err := lib.EvalFromSource(nil, src)
		require.NoError(t, err)
		require.True(t, len(ret) == 0, "single element should have no duplicates")
	})

	t.Run("multiple unique elements", func(t *testing.T) {
		// Multiple unique elements - no duplicates
		tup := tuples.MakeTupleFromDataElements([]byte{1}, []byte{2}, []byte{3})
		src := fmt.Sprintf("tupleHasDuplicates(0x%s)", hex.EncodeToString(tup.Bytes()))
		ret, err := lib.EvalFromSource(nil, src)
		require.NoError(t, err)
		require.True(t, len(ret) == 0, "unique elements should have no duplicates")
	})

	t.Run("duplicates at start", func(t *testing.T) {
		// Duplicates at start - returns truthy
		tup := tuples.MakeTupleFromDataElements([]byte{1}, []byte{1}, []byte{3})
		src := fmt.Sprintf("tupleHasDuplicates(0x%s)", hex.EncodeToString(tup.Bytes()))
		ret, err := lib.EvalFromSource(nil, src)
		require.NoError(t, err)
		require.True(t, len(ret) != 0, "should detect duplicates at start")
	})

	t.Run("duplicates at end", func(t *testing.T) {
		// Duplicates at end
		tup := tuples.MakeTupleFromDataElements([]byte{1}, []byte{2}, []byte{1})
		src := fmt.Sprintf("tupleHasDuplicates(0x%s)", hex.EncodeToString(tup.Bytes()))
		ret, err := lib.EvalFromSource(nil, src)
		require.NoError(t, err)
		require.True(t, len(ret) != 0, "should detect duplicates at end")
	})

	t.Run("adjacent duplicates", func(t *testing.T) {
		// Adjacent duplicate multi-byte elements
		tup := tuples.MakeTupleFromDataElements([]byte{1, 2}, []byte{1, 2}, []byte{3})
		src := fmt.Sprintf("tupleHasDuplicates(0x%s)", hex.EncodeToString(tup.Bytes()))
		ret, err := lib.EvalFromSource(nil, src)
		require.NoError(t, err)
		require.True(t, len(ret) != 0, "should detect adjacent duplicates")
	})

	t.Run("all same elements", func(t *testing.T) {
		// All elements are the same
		tup := tuples.MakeTupleFromDataElements([]byte{5}, []byte{5}, []byte{5})
		src := fmt.Sprintf("tupleHasDuplicates(0x%s)", hex.EncodeToString(tup.Bytes()))
		ret, err := lib.EvalFromSource(nil, src)
		require.NoError(t, err)
		require.True(t, len(ret) != 0, "should detect all same elements as duplicates")
	})

	t.Run("nil elements no duplicates", func(t *testing.T) {
		// Nil element mixed with other elements - no duplicates
		tup := tuples.MakeTupleFromDataElements(nil, []byte{1}, []byte{2})
		src := fmt.Sprintf("tupleHasDuplicates(0x%s)", hex.EncodeToString(tup.Bytes()))
		ret, err := lib.EvalFromSource(nil, src)
		require.NoError(t, err)
		require.True(t, len(ret) == 0, "nil with unique elements should have no duplicates")
	})

	t.Run("nil elements with duplicates", func(t *testing.T) {
		// Two nil elements - are duplicates
		tup := tuples.MakeTupleFromDataElements(nil, []byte{1}, nil)
		src := fmt.Sprintf("tupleHasDuplicates(0x%s)", hex.EncodeToString(tup.Bytes()))
		ret, err := lib.EvalFromSource(nil, src)
		require.NoError(t, err)
		require.True(t, len(ret) != 0, "two nil elements are duplicates")
	})

	t.Run("empty byte slices are duplicates", func(t *testing.T) {
		// Two empty byte slices are considered duplicates
		tup := tuples.MakeTupleFromDataElements([]byte{}, []byte{1}, []byte{})
		src := fmt.Sprintf("tupleHasDuplicates(0x%s)", hex.EncodeToString(tup.Bytes()))
		ret, err := lib.EvalFromSource(nil, src)
		require.NoError(t, err)
		require.True(t, len(ret) != 0, "empty byte slices are duplicates")
	})

	t.Run("invalid tuple data", func(t *testing.T) {
		// Invalid tuple data should cause an error/panic
		lib.MustError("tupleHasDuplicates(0xff)", "")
	})

	t.Run("using MustTrue and MustEqual patterns", func(t *testing.T) {
		// No duplicates - should be false/nil
		noDups := tuples.MakeTupleFromDataElements([]byte{1}, []byte{2}, []byte{3})
		noDupsSrc := fmt.Sprintf("tupleHasDuplicates(0x%s)", hex.EncodeToString(noDups.Bytes()))
		lib.MustEqual(noDupsSrc, "false")

		// With duplicates - should be truthy
		withDups := tuples.MakeTupleFromDataElements([]byte{1}, []byte{2}, []byte{1})
		withDupsSrc := fmt.Sprintf("tupleHasDuplicates(0x%s)", hex.EncodeToString(withDups.Bytes()))
		lib.MustTrue(withDupsSrc)
	})

	t.Run("large tuple no duplicates", func(t *testing.T) {
		// Test with more elements
		elements := make([][]byte, 100)
		for i := range elements {
			elements[i] = []byte{byte(i)}
		}
		tup := tuples.MakeTupleFromDataElements(elements...)
		src := fmt.Sprintf("tupleHasDuplicates(0x%s)", hex.EncodeToString(tup.Bytes()))
		ret, err := lib.EvalFromSource(nil, src)
		require.NoError(t, err)
		require.True(t, len(ret) == 0, "100 unique elements should have no duplicates")
	})

	t.Run("large tuple with duplicates", func(t *testing.T) {
		// Test with more elements that have duplicates
		elements := make([][]byte, 100)
		for i := range elements {
			elements[i] = []byte{byte(i)}
		}
		elements[99] = []byte{0} // duplicate of first element
		tup := tuples.MakeTupleFromDataElements(elements...)
		src := fmt.Sprintf("tupleHasDuplicates(0x%s)", hex.EncodeToString(tup.Bytes()))
		ret, err := lib.EvalFromSource(nil, src)
		require.NoError(t, err)
		require.True(t, len(ret) != 0, "should detect duplicate in large tuple")
	})
}
